...
// 
/**\class EcalCalibrationWenu EcalCalibrationWenu.cc EcalValidation/EcalCalibrationWenu/src/EcalCalibrationWenu.cc
...
#include "EcalValidation/EcalCalibrationWenu/interface/EcalCalibrationWenu.h"
...
//
EcalCalibrationWenu::EcalCalibrationWenu(const edm::ParameterSet& iConfig){
  ...
  myTree_ -> Branch("BXId",&BXId_,"BXId/I");
...
EcalCalibrationWenu::~EcalCalibrationWenu()
{
...
void
EcalCalibrationWenu::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
...
///==== get Event information ====
 ...
 for ( unsigned int i=0; i<electrons.size(); ++i ){
   ...
   ///==== Selections inline to optimiza time ====
...
//    if (fabs(eta_) <= EtaCutEB && DeltaEtaIn_ > 0.004) continue;
 ...
 if ( nGoodElectrons_ == 1 ) {//==== Wenu -> !1 electron
// if ( nGoodElectrons_ != 0 ) {//==== Wenu -> !1 electron

  pat::Electron electron = electrons.at(chosenEle);
  reco::GsfTrackRef eleTrack  = electron.gsfTrack () ; 
  reco::SuperClusterRef scRef = electron.superCluster();
...
void 
EcalCalibrationWenu::beginJob()
{
}

// ------------ method called once each job just after ending the event loop  ------------
void 
EcalCalibrationWenu::endJob() {
}

//define this as a plug-in
DEFINE_FWK_MODULE(EcalCalibrationWenu);
