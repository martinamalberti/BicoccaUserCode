...
MCDumperHiggs::MCDumperHiggs(edm::Handle<reco::GenParticleCollection>& genParticles,
 ...
 {
  if(verbosity_p)
    std::cerr << "*** MCDumperHiggs::ctor ***" << std::endl;
...
MCDumperHiggs::~MCDumperHiggs()
{
  if(verbosity_p)
    std::cerr << "*** MCDumperHiggs::dtor ***" << std::endl;
}
...
void MCDumperHiggs::Analyze(edm::Handle<reco::GenParticleCollection>& genParticles)
{
  if(verbosity_p)
    std::cerr << "*** MCDumperHiggs::Analyze ***" << std::endl;
      ...
      p != genParticles -> end(); ++p)
  {
    ...
    // higgs
    if( ( (pdgId == 25) || (pdgId == 39) || (pdgId == 5000039) ) && (status == 3) )
    {
      mcH_p = pCurrent;
    }
...
void MCDumperHiggs::PrintEventInfo()
{
  std::cout << "\n>>> MCDumperHiggs::PrintHInfo \n" << std::endl;
...
void MCDumperHiggs::GetHDecayMode()
{
  ...
  if( (mcV1_p -> pdgId() == 22) && (mcV2_p -> pdgId() == 22) )
    mcHDecayMode_p = "gamma_gamma";
}
...
void MCDumperHiggs::GetVDecayMode(const int& Vindex)
{
