              ...
              TH1F* histo)
{
  ...
  (*fitFunc_voigtian) = new TF1((fitFuncName+"_voigtian").c_str(),myTripleVoigt,gaus_mean-3.*gaus_sigma,gaus_mean+3.*gaus_sigma,9);
  ...
  (*fitFunc_crystalBall) = new TF1((fitFuncName+"_crystalBall").c_str(),breitWigner_crystalBallLow,gaus_mean-3.*gaus_sigma,gaus_mean+3.*gaus_sigma,7);
...
void GetOLSigma(double& peak, double& sigmaOL,
                TF1* f, const double& prec, TArrow** line)
{
