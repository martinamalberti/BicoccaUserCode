...
int main () 
{
  ...
  EcalIntercalibConstants::EcalIntercalibConstantMap imap = constants->getMap () ;
  ...
  EcalIntercalibConstants::EcalIntercalibConstantMap imap2 = constants2->getMap () ;
  for (int x=1;x<=100;++x)
   for (int y=1;y<100;++y)
   {
     if (!EEDetId::validDetId(x,y,-1)) continue;
     EEDetId ee (x,y,-1,EEDetId::XYMODE);
     endcapWriter.writeLine (ee, (imap.find(ee.rawId()))->second*(imap2.find(ee.rawId()))->second); 
     if (!EEDetId::validDetId(x,y,1)) continue;
     EEDetId e2 (x,y,1,EEDetId::XYMODE);
     endcapWriter.writeLine (e2, (imap.find(e2.rawId()))->second*(imap2.find(e2.rawId()))->second); 
   }
}
