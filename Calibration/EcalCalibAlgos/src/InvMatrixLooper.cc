...
//!LP ctor
InvMatrixLooper::InvMatrixLooper (const edm::ParameterSet& iConfig) :
...
{
   edm::LogInfo ("IML") << "[InvMatrixLooper][ctor] asserts" ;
...
//        assert (m_phiEndEE >=0); // m_phiStartEE || m_phiEndEE < 360);
   ...
   edm::LogInfo ("IML") << "[InvMatrixLooper][ctor] entering " ;
...
int index;
   ...
   edm::LogInfo ("IML") << "[InvMatrixLooper][ctor] region definition" ;
   EBRegionDefinition () ;
   EERegionDefinition () ;
///Graphs to ckeck the region definition
  ...
  edm::LogInfo ("IML") << "[InvMatrixLooper][ctor] Calib Block" ;

  for (int region = 0; region < EBregionsNum ()+2*EEregionsNum() ; region++)
           m_ecalCalibBlocks.push_back (EcalCalibBlock (m_regions.at (region))); 
 } //end ctor
...
//!LP destructor
InvMatrixLooper::~InvMatrixLooper ()
{
  edm::LogInfo ("IML") << "[InvMatrixLooper][dtor]" ;
}
...
//!BeginOfJob
void 
InvMatrixLooper::beginOfJob (const edm::EventSetup & iSetup) 
{
  edm::LogInfo ("IML") << "[InvMatrixLooper][beginOfJob]" ;
...
void InvMatrixLooper::startingNewLoop (unsigned int ciclo) 
{
...
edm::EDLooper::Status
InvMatrixLooper::duringLoop (const edm::Event& iEvent,
...
edm::EDLooper::Status InvMatrixLooper::endOfLoop (const edm::EventSetup& dumb,unsigned int iCounter)
{
...
int 
InvMatrixLooper::EBregionCheck (const int eta, const int phi) const 
 {
...
double InvMatrixLooper::giveLimit (int degrees)
  {
    //PG 200 > atan (50/0.5)
...
int InvMatrixLooper::EBRegionId (const int etaXtl,const int phiXtl) const 
{
...
int InvMatrixLooper::EERegionId (const int ics, const int ips) const
{
...
inline int InvMatrixLooper::EEregionsNum () const 
{
  int phifake = m_phiStartEE;
  if (m_phiStartEE>m_phiEndEE) phifake = m_phiStartEE - 360;
...
inline int InvMatrixLooper::EBregionsNum () const 
{
...
void InvMatrixLooper::EBRegionDefinition ()
{
 int reg=-1;
...
void InvMatrixLooper::EERegionDefinition ()
{
 // reset
...
int InvMatrixLooper::EEregionCheck (const int ics, const int ips)  const
{
  int x = ics-50;
  int y = ips-50;
...
inline int InvMatrixLooper::etaShifter (const int etaOld) const
   {
...
DetId  InvMatrixLooper::findMaxHit (const std::vector<DetId> & v1,
...
//---------------------------------------------------
//!Fills the map to be sent to the EcalCalibBlock

void InvMatrixLooper::fillEBMap (EBDetId EBmax,
...
void InvMatrixLooper::fillEEMap (EEDetId EEmax,
