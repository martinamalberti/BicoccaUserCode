...
void trisCheckCorrection(Char_t* EBEE = 0,
                         ...
                         float laserCorrMax = -1.)
{
  ...
  float scLaserCorr, scCrackCorrection;
  float EoP;
  ...
  for(int ientry = 0; ientry < nEntries; ++ientry)
  {
    ...
    //Ediff -> Fill( ( (scCrackCorrection*fClusterCorrections(scERaw+ES,scEta,scPhiWidth/scEtaWidth,params))-scE)/scE );
    //Ediff_vsEta -> Fill( scEta, ( (ES+scCrackCorrection*fClusterCorrections(scERaw,scEta,scPhiWidth/scEtaWidth,params))-scE)/scE );
  ...
  if ( strcmp(EBEE,"EB")==0 )
  {  
    h_EoP_spread = new TH1F("h_EoP_spread","",100,0.960,1.010);
    h_EoC_spread = new TH1F("h_EoC_spread","",100,0.960,1.010);    
  }
  else 
  {  
    h_EoP_spread = new TH1F("h_EoP_spread","",100,0.840,1.040);
    h_EoC_spread = new TH1F("h_EoC_spread","",100,0.840,1.040);    
  }
  ...
  for(int ientry = 0; ientry < nEntries; ++ientry)
  {
    ...
    float scale = 1.;
    //scale = sqrt( pow(scLaserCorr,((1.52-0.7843)/1.52)-1.) );
    //scale = 1. / (0.1811 + 0.7843*scLaserCorr);
  ...
  for(int i = 0; i < nBins; ++i)
  {
    ...
    {
      g_fit -> SetPoint(i,  h_Las[i]->GetMean() , 1./f_EoP[i]->GetParameter(1));
    ...
    if (fStatus!=4 && eee>0.1*h_template->GetRMS()/sqrt(evtsPerPoint))
    {        
      g_c_fit -> SetPoint(i,  h_Las[i]->GetMean() , 1./f_EoC[i]->GetParameter(1));
  ...
  TF1* pol0 = new TF1("pol0","pol0",lcMin,lcMax);
  ...
  */
  ...
  if ( strcmp(EBEE,"EB")==0 )
  {  
    hPad -> SetMinimum(0.960);
    hPad -> SetMaximum(1.010);
  }
  else 
  {  
    hPad -> SetMinimum(0.840);
    hPad -> SetMaximum(1.040);
  }
