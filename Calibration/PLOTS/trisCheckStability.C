                        ...
                        std::string dayMax = "")
{
  ...
  int timeStampHigh, timeStampLow;
  int isW, isZ;
  float scLaserCorr, EoP, scEta;
  int seedIeta, seedIphi, seedIx, seedIy, seedZside;
  
  ntu_DA->SetBranchAddress("timeStampHigh", &timeStampHigh);
  ntu_DA->SetBranchAddress("timeStampLow", &timeStampLow);
  ...
  if ( strcmp(EBEE,"EB")==0 )
  {  
    h_EoP_spread = new TH1F("h_EoP_spread","",100,0.970,1.010);
    h_EoC_spread = new TH1F("h_EoC_spread","",100,0.970,1.010);    
  }
  else 
  {  
    h_EoP_spread = new TH1F("h_EoP_spread","",100,0.910,1.030);
    h_EoC_spread = new TH1F("h_EoC_spread","",100,0.910,1.030);
  }
