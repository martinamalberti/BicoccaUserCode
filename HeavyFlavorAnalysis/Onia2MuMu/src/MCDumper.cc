#include "HeavyFlavorAnalysis/Onia2MuMu/interface/MCDumper.h"
...
void MCDumper::Analyze(edm::Handle<reco::GenParticleCollection>& genParticles)
{
  ...
  if(!mcH_p)
  {
    std::cerr << ">>> MCDumper::Analyze::Warning: no H in the event" << std::endl;
  ...
  if(VBuffer.size() != 2)
  {
     std::cerr << "MCDumper::Analyze::Warning: not a H -> VV decay" << std::endl;
  ...
  {
     std::cerr << "MCDumper::Analyze::Warning: not a V1 -> ff decay" << std::endl;
  ...
  {
     std::cerr << "MCDumper::Analyze::Warning: not a V2 -> ff decay" << std::endl;
...
void MCDumper::GetVDecayMode(const int& Vindex)
{
