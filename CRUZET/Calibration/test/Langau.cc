...
//  TFile *_file0 = TFile::Open("HistoGlobalAngleCut.root");
...
//    fr[1]=3.0*hSNR->GetMean();
 ...
 pllo[0]=0.00001; pllo[1]=0.0000001; pllo[2]=0.001; pllo[3]=0.000001;
 plhi[0]=1.0; plhi[1]=1.0; plhi[2]=100000.0; plhi[3]=1.;
 sv[0]=0.0041; sv[1]=0.0118; sv[2]=0.5; sv[3]=.00001;
 ...
 sv[2] = 1000. / dEdx_DLB->GetEntries();
 TF1 *fitsnr = langaufit(&dEdx_DLB,fr,sv,pllo,plhi,fp,fpe,&chisqr,&ndf);
 /*TF1 *langaufit(TH1F *his, Double_t *fitrange, Double_t *startvalues, Double_t *parlimitslo, Double_t *parlimitshi, Double_t *fitparams, Double_t *fiterrors, Double_t *ChiSqr, Int_t *NDF)*/
 ...
 dEdx_DLB->GetXaxis()->SetRange(0.,0.2);
 dEdx_DLB->Draw();
 fitsnr->Draw("lsame");
 ...
 sv[2] = 1000. / dEdx_DRB->GetEntries();
 TF1 *fitsnr = langaufit(&dEdx_DRB,fr,sv,pllo,plhi,fp,fpe,&chisqr,&ndf);
 /*TF1 *langaufit(TH1F *his, Double_t *fitrange, Double_t *startvalues, Double_t *parlimitslo, Double_t *parlimitshi, Double_t *fitparams, Double_t *fiterrors, Double_t *ChiSqr, Int_t *NDF)*/
 ...
 dEdx_DRB->GetXaxis()->SetRange(0.,0.2);
 dEdx_DRB->Draw();
 fitsnr->Draw("lsame");
 ...
 sv[2] = 1000. / dEdx_DRF->GetEntries();
 TF1 *fitsnr = langaufit(&dEdx_DRF,fr,sv,pllo,plhi,fp,fpe,&chisqr,&ndf);
 langaupro(fp,SNRPeak,SNRFWHM);
 ...
 dEdx_DRF->GetXaxis()->SetRange(0.,0.2);
 dEdx_DRF->Draw();
 fitsnr->Draw("lsame");
 ...
 Pippo->cd(4);
 dEdx_DLF->Rebin(8);
 sv[2] = 1000. / dEdx_DLF->GetEntries();
 TF1 *fitsnr = langaufit(&dEdx_DLF,fr,sv,pllo,plhi,fp,fpe,&chisqr,&ndf);
 langaupro(fp,SNRPeak,SNRFWHM);
 ...
 dEdx_DLF->GetXaxis()->SetRange(0.,0.2);
 dEdx_DLF->Draw();
 fitsnr->Draw("lsame");
 ...
 Uno->Divide(2,2);
 Uno->cd(1);
 dE_DLB->SetFillColor(kRed);
 dE_DLB->Rebin(8);
 dE_DLB->Draw();
   
 Uno->cd(2);
 dE_DRB->SetFillColor(kRed);
 dE_DRB->Rebin(8);
 dE_DRB->Draw();
   
 Uno->cd(3);
 dE_DLF->SetFillColor(kRed);
 dE_DLF->Rebin(8);
 dE_DLF->Draw();
   
 Uno->cd(4);
 dE_DRF->SetFillColor(kRed);
 dE_DRF->Rebin(8);
 dE_DRF->Draw();
 ...
 TCanvas *Tre  = new TCanvas();
 Tre->Divide(2,2);
 Tre->cd(1);
 dE_ULB->SetFillColor(kBlue);
 dE_ULB->Rebin(8);
 dE_ULB->Draw();
   
 Tre->cd(2);
 dE_URB->SetFillColor(kBlue);
 dE_URB->Rebin(8);
 dE_URB->Draw();
   
 Tre->cd(3);
 dE_ULF->SetFillColor(kBlue);
 dE_ULF->Rebin(8);
 dE_ULF->Draw();
   
 Tre->cd(4);
 dE_URF->SetFillColor(kBlue);
 dE_URF->Rebin(8);
 dE_URF->Draw();
 ...
 TCanvas *Due  = new TCanvas();
 Due->Divide(2,2);
 Due->cd(1);
 CoeffDR->SetMarkerColor(kRed);
 CoeffDR->SetMarkerSize(0.5);
 CoeffDR->SetMarkerStyle(20);
 CoeffDR->Draw("ap");
   
 Due->cd(2);
 CoeffDL->SetMarkerColor(kRed);
 CoeffDL->SetMarkerSize(0.5);
 CoeffDL->SetMarkerStyle(20);
 CoeffDL->Draw("ap");
   
 Due->cd(3);
 CoeffUR->SetMarkerColor(kBlue);
 CoeffUR->SetMarkerSize(0.5);
 CoeffUR->SetMarkerStyle(20);
 CoeffUR->Draw("ap");
   
 Due->cd(4);
 CoeffUL->SetMarkerColor(kBlue);
 CoeffUL->SetMarkerSize(0.5);
