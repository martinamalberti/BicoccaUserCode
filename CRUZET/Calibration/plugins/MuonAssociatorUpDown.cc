...
MuonAssociatorUpDown::MuonAssociatorUpDown (const edm::ParameterSet& iConfig):
...
void MuonAssociatorUpDown::analyze ( const edm::Event & iEvent, const  edm::EventSetup& iSetup)
{
  ...
  if (recoTracks->size() != 2 ) return ;                                     //chiedo eventi a 2 tracce

  TrajectoryStateTransform tsTransform;
  edm::ESHandle<MagneticField> bField;
  iSetup.get<IdealMagneticFieldRecord>().get(bField);

  //------------chiamo fuori dal loop sulle tracce------------------
  ...
  for(reco::TrackCollection::const_iterator recoTrack = recoTracks->begin();
    ...
    { 
      ...
      if(innerState.position().y() < 0)       //prendo i punti più esterni
	{
	  sotto = true;
  ...
  //AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAABBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
  if(sopra == false || sotto == false) return;
  //if(mag1 == mag2) return;  //è lo stesso punto!!! non so tirare una retta.  (direi impossibile)
  ...
  if(pOut1.mag() < 0.01 || pOut2.mag() < 0.01) return;
  if(pIn1.mag() < 0.01 || pIn2.mag() < 0.01) return;  

  double lunghTop = getEcalXLenght(pIn1,pOut1);
  double lunghBot = getEcalXLenght(pIn2,pOut2);

  //std::cerr << "point1 = " << point1.mag() << ";   point2 = " << point2.mag() << std::endl;
  //std::cerr << "pIn1 = " << pIn1.mag() << ";   pIn2 = " << pIn2.mag() << ";   pOut1 = " << pOut1.mag() << "; pOut2 = " << pOut2.mag() << std::endl;
  //std::cerr << "-----> lungh segmento top = " << lunghTop << ";  lungh segmento bottom =  " << lunghBot << endl;
  //std::cerr << std::endl;
  ...
  if(pIn1.eta() > 0) GPointPos++;
  else if (pIn1.eta() < 0) GPointNeg++;

  if(pIn2.eta() > 0) GPointPos++;
  else if (pIn2.eta() < 0) GPointNeg++;
...
//     {
//       deta1 = fabs( (hitCrystals.at(ii).ieta()-0.5)*0.01745 - pIn1.eta());     
//       dphi1 = fabs( (hitCrystals.at(ii).iphi()-0.5)*0.01745 - pIn1.phi());  //  - twopi/2
//       if (dphi1>(twopi-dphi1)) {dphi1 = twopi-dphi1;}
      
//       deta2 = fabs( (hitCrystals.at(ii).ieta()-0.5)*0.01745 - pIn2.eta());         
//       dphi2 = fabs( (hitCrystals.at(ii).iphi()-0.5)*0.01745 - pIn2.phi());  //  - twopi/2
...
//       if(info.crossedEcalIds.size()==0) return;
...
// 	  //------------- distanza DetID-----------------------
// 	  detaDetID1 = fabs( (dummy.ieta() - 0.5) * 0.01745 - pIn1.eta());       //non è necessaria la correzione!!
// 	  if((dummy.iphi() - 0.5) * 0.01745 > twopi/2) dphiDetID1 = fabs((dummy.iphi() - 0.5) * 0.01745 - twopi - pIn1.phi());
// 	  else dphiDetID1 = fabs( (dummy.iphi() - 0.5) * 0.01745 - pIn1.phi());
// 	  if (dphiDetID1 > (twopi-dphiDetID1)) {dphiDetID1 = twopi-dphiDetID1;}
	  
// 	  detaDetID2 = fabs( (dummy.ieta() - 0.5) * 0.01745 - pIn2.eta()); 
// 	  if((dummy.iphi() - 0.5) * 0.01745 > twopi/2) dphiDetID2 = fabs((dummy.iphi() - 0.5) * 0.01745 - twopi - pIn2.phi());
// 	  else dphiDetID2 = fabs( (dummy.iphi() - 0.5) * 0.01745 - pIn2.phi());
...
void 
MuonAssociatorUpDown::beginJob(const edm::EventSetup&)
{
...
//-------------deltaPhi--------------------
int MuonAssociatorUpDown::deltaPhi (int phi1, int phi2)
{
...
//------------------------------------------------------------
std::vector<double>
MuonAssociatorUpDown::getClusterEnergy (const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  int ievt = iEvent.id().event();
  edm::Handle<reco::BasicClusterCollection> bccHandle;  
  edm::Handle<reco::BasicClusterCollection> eccHandle;  
...
MuonAssociatorUpDown::getEcalCross(const GlobalPoint& G1,const GlobalPoint& G2,const double& ECALrad)
{
...
double 
MuonAssociatorUpDown::getEcalXLenght (const GlobalPoint& G1,const GlobalPoint& G2)
{
