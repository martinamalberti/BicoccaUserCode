// -*- C++ -*-
//
// Package:   EcalCosmicsTreeTest 
// Class:     EcalCosmicsTreeTest  
// 
/**\class EcalCosmicsTreeTest EcalCosmicsTreeTest.cc
...
//
EcalCosmicsTreeTest::EcalCosmicsTreeTest(const edm::ParameterSet& iConfig) :
...
EcalCosmicsTreeTest::~EcalCosmicsTreeTest()
{
}
...
void
EcalCosmicsTreeTest::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
...
//       }
  ...
  if (!(bccHandle.isValid())) 
    {
      LogWarning("EcalCosmicsTreeTest") << barrelClusterCollection_ << " not available";
      return; 
    }
  LogDebug("EcalCosmicsTreeTest") << "event " << ievt;
  
  iEvent.getByLabel(endcapClusterCollection_, eccHandle);
  if (!(eccHandle.isValid())) 
    {
      LogWarning("EcalCosmicsTreeTest") << endcapClusterCollection_ << " not available";
      //return;
    }
  ...
  if (!(eccHandle.isValid())) 
    {
      LogWarning("EcalCosmicsTreeTest") << ecalRecHitCollection_ << " not available";
      //return;
    }
  ...
  if (!(gmtrc_handle.isValid())) 
    {
	  LogWarning("EcalCosmicsTreeTest") << "l1MuGMTReadoutCollection" << " not available";
      //return;
    }
  ...
  if (!(L1GTRR.isValid()))
    {
      LogWarning("EcalCosmicsTreeTest") << l1GTReadoutRecTag_ << " not available";
      //return;
    }
  else if(sizeOfDecisionWord<128)
    {
      LogWarning("EcalCosmicsTreeTest") << "size of L1 decisionword is " << sizeOfDecisionWord
  ...
  */
  ...
  if ( !recoTracks.isValid() ) {
    LogWarning("EcalCosmicsTreeTest") << "... No Valid Muon recoTracks !!! " << recoTracks.product()->size();
  }
...
std::vector<GlobalPoint> EcalCosmicsTreeTest::getTrajectoryPoints( const GlobalPoint& G1, const GlobalPoint& G2,const float& dt  )
{
...
std::vector<DetId> EcalCosmicsTreeTest::matrixDetId( const CaloTopology* topology, DetId id, int ixMin, int ixMax, int iyMin, int iyMax )
{
...
std::string EcalCosmicsTreeTest::intToString(int num)
{
    using namespace std;
