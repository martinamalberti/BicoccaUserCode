...
//  }
//  
//  TChain *chain = new TChain ("AlCaValidationTEST/SimpleNtple") ;
//   
//  if ( ff == 0 ){
...
//       
//    float pt = reader.Get3V("electrons_tracker_atVtx")->at(iEle).Rho();
//    p  = reader.Get3V("electrons_tracker_atVtx")->at(iEle).R();
//    float et = reader.Get4V("electrons")->at(iEle).Et();
//       
//    met  = reader.Get4V("met")->at(0).Et();
// 
//    float cphi = (reader.Get3V("electrons_tracker_atVtx")->at(iEle).x() * reader.Get4V("met")->at(0).Px() 
//      + reader.Get3V("electrons_tracker_atVtx")->at(iEle).y() * reader.Get4V("met")->at(0).Py()) 
//      / (met*pt);
// 
//    
//    float mt   = sqrt(2*et*met*(1-cphi));
// 
//    float dphi = deltaPhi(reader.Get4V("met")->at(0).Phi(),reader.Get4V("electrons")->at(iEle).phi());
// 
//    float sumEt = 0;
...
// //    }
// 
//    sumEt = reader.GetFloat("sumEt")->at(0);
//    
...
// //    (1. - E4/E1)
//    if ((1. - reader.GetFloat("SwissE4")->at(iEle)/reader.GetFloat("electrons_eSeed")->at(iEle)) > 0.95) continue;
// 
...
//   
//   float pt = reader.Get3V("electrons_tracker_atVtx")->at(chosenEle).Rho();
//   p  = reader.Get3V("electrons_tracker_atVtx")->at(chosenEle).R();
//     
//   float et = reader.Get4V("electrons")->at(chosenEle).Et();
//     
//   met  = reader.Get4V("met")->at(0).Et();
//   
//   float cphi = (reader.Get3V("electrons_tracker_atVtx")->at(chosenEle).x() * reader.Get4V("met")->at(0).Px() 
//     + reader.Get3V("electrons_tracker_atVtx")->at(chosenEle).y() * reader.Get4V("met")->at(0).Py()) 
//      / (met*pt);
// 
//   float mt   = sqrt(2*et*met*(1-cphi));
//     
//   float dphi = deltaPhi(reader.Get4V("met")->at(0).Phi(),reader.Get4V("electrons")->at(chosenEle).phi());
// 
//   float sumEt = 0;
// //   for (unsigned int iJet=0; iJet<reader.Get4V("jets")->size(); iJet++){
// //    float deta = reader.Get4V("jets")->at(iJet).Eta() -  eta;
...
//     heopEBscaled[i]->Fill(alpha * reader.Get4V("electrons")->at(chosenEle).E() /p ,w);
//    }
//       
//       // EE
...
//     heopEEscaled[i]->Fill(alpha * reader.Get4V("electrons")->at(chosenEle).E() /p ,w);
//    }
//   }
...
//   float EtaCutEE    = 1.560;
 ...
 for (int entry = 0; entry < nEntries; ++entry) {
  ...
  for (int iEle = 0; iEle < nEles; iEle++){
   ...
   pt = (reader.GetFloat("electrons_pin")->at(iEle))*(reader.Get4V("electrons")->at(iEle).Pt())/(reader.Get4V("electrons")->at(iEle).P());
   p  = reader.GetFloat("electrons_pin")->at(iEle);
   et = reader.Get4V("electrons")->at(iEle).Et();
...
//    if ( treeVars.eleId[i] != 15 ) continue;
   ...
   eta = reader.Get4V("electrons")->at(chosenEle).eta();
   pt = (reader.GetFloat("electrons_pin")->at(chosenEle))*(reader.Get4V("electrons")->at(chosenEle).Pt())/(reader.Get4V("electrons")->at(chosenEle).P()); 
   p  = reader.GetFloat("electrons_pin")->at(chosenEle);
   met  = reader.Get4V("CALOMet")->at(0).Et();
   sumEt = reader.GetFloat("CALOSumEt")->at(0);
   et = reader.Get4V("electrons")->at(chosenEle).Et();
