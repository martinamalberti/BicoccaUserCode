void Draw_InvMass () {
  ...
  v_signal_names.push_back("Signal_8GeV_samesign");
  v_signal_names.push_back("Signal_10GeV_samesign") ; 
  
  v_leg_signal_names.push_back("a_8GeV") ; 
  v_leg_signal_names.push_back("a_10GeV") ;
  
  v_bkg_SM_names.push_back("MuMuBkg_samesign") ;
  v_bkg_SM_names.push_back("Upsilon1S_samesign") ;
  v_bkg_SM_names.push_back("Upsilon2S_samesign") ;
  v_bkg_SM_names.push_back("Upsilon3S_samesign") ;
  ...
  double signal_Xsec [n_signal] = { 1893, 4537.5};// <= values for tanBeta = 10,  values for tanBeta = 3 => {94.64 151.25}  ; 
  double bkg_SM_Xsec [n_bkg_SM] = { 85000000., 14000., 5600., 1600., 85000000. } ;
  
  double signal_filter_eff [n_signal] = { 1., 1. } ;
  double bkg_SM_filter_eff [n_bkg_SM] = { 1., 1., 1., 1., 1. } ;

  double signal_genMCevts [n_signal] = { 9600., 9600. } ; 
  double bkg_SM_genMCevts [n_bkg_SM] = {  800000., 10000., 10000., 10000., 800000. } ;
...
/*    can[i]->SetGridx () ;
  ...
  //SM BKG
    ...
    leg [i] ->Draw() ;
    std::cout << Lum << endl;
    s_plot_name = "~/www/little" + v_leg_signal_names [i] +"_" + Lum + "pb_" + "tanB"+ tanB + "samesign.gif" ;

    can [i] -> Print ( s_plot_name, "gif" ) ;
    s_plot_name = "~/www/little" + v_leg_signal_names [i] + "_" + Lum + "pb_" + "tanB"+ tanB +"samesign.eps" ;
