...
//     double lumi = 5.1e-3;	//lumiNew
  ...
  double xsec1Herwig = 714000000. / 100000;
  double xsec2Herwig = 714000000.  / 100000;
...
//   double xsecPowheg = 900336751.9368 / 998347.;
  ...
  TTree* TreeMineMC = (TTree*) FileMineMC.Get("AnaHiggs");
 ...
 TreeTrain1Herwig->SetBranchAddress("G_FJet_Pt",&G_FJet_Pt_P);
 TreeTrain1Herwig->SetBranchAddress("S_FJet_Pt",&S_FJet_Pt_P);
 TreeTrain1Herwig->SetBranchAddress("G_CJet_Pt",&G_CJet_Pt_P);
 TreeTrain1Herwig->SetBranchAddress("S_CJet_Pt",&S_CJet_Pt_P);

 TreeTrain2Herwig->SetBranchAddress("G_FJet_Pt",&G_FJet_Pt_P);
 TreeTrain2Herwig->SetBranchAddress("S_FJet_Pt",&S_FJet_Pt_P);
 TreeTrain2Herwig->SetBranchAddress("G_CJet_Pt",&G_CJet_Pt_P);
 TreeTrain2Herwig->SetBranchAddress("S_CJet_Pt",&S_CJet_Pt_P);
 
 TreeTrainPOW->SetBranchAddress("G_FJet_Pt",&G_FJet_Pt_P);
 TreeTrainPOW->SetBranchAddress("G_CJet_Pt",&G_CJet_Pt_P);
 
 TreeMineMC->SetBranchAddress("G_FJet_Pt",&G_FJet_Pt_P);
 TreeMineMC->SetBranchAddress("G_CJet_Pt",&G_CJet_Pt_P);
...
//    hResponseMatrixCJet->SetBinContent(iBinX+1,iBinY+1,hResponseMatrixCJet->GetBinContent(iBinX+1,iBinY+1) / xsecScale ) ;
  ...
  for (Int_t iEvt= 0; iEvt<TreeTrain1Herwig->GetEntries(); iEvt++) {
   TreeTrain1Herwig->GetEntry(iEvt);
   //    std::cerr << " S_FJet_Pt = " << S_FJet_Pt << std::endl;
   if (S_FJet_Pt_P>0 && S_CJet_Pt_P>0 && G_CJet_Pt_P>0 && G_FJet_Pt_P) {
    if (G_FJet_Pt_P < Threshold_G_FJet) {
     hTrueFJet_MC_tempHerwig->Fill(G_FJet_Pt_P);
     hMeasFJet_MC_tempHerwig->Fill(S_FJet_Pt_P);
    }
    if (G_CJet_Pt_P < Threshold_G_CJet) {
     hTrueCJet_MC_tempHerwig->Fill(G_CJet_Pt_P);
     hMeasCJet_MC_tempHerwig->Fill(S_CJet_Pt_P);
    }
   }
  }
  ...
  for (Int_t iEvt= 0; iEvt<TreeTrain2Herwig->GetEntries(); iEvt++) {
   ...
   if (S_FJet_Pt_P>0 && S_CJet_Pt_P>0 && G_CJet_Pt_P>0 && G_FJet_Pt_P) {
    if (G_FJet_Pt_P > Threshold_G_FJet) {
     hTrueFJet_MCHerwig->Fill(G_FJet_Pt_P);
     hMeasFJet_MCHerwig->Fill(S_FJet_Pt_P);
    }
    if (G_CJet_Pt_P > Threshold_G_CJet) {
     hTrueCJet_MCHerwig->Fill(G_CJet_Pt_P);
     hMeasCJet_MCHerwig->Fill(S_CJet_Pt_P);
    }
   }
  }  
