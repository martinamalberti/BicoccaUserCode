...
//   TDRStyle();
 ...
 try {
  ...
  int slimNBIN = 10*v_lowEdge.size() -1;
  int thickness = 10;
  

  Float_t* lowEdgeSlim = new Float_t [10*v_lowEdge.size()];
  
  for (int iBin = 0; iBin<NBIN; iBin++) {
    
   double width = (v_lowEdge[iBin+1] - v_lowEdge[iBin])/thickness;
   
   for (int i = 0; i<thickness; i++){
//      v_lowEdgeSlim.push_back(v_lowEdge[iBin] + i*width);
	lowEdgeSlim[iBin*10 + i] = v_lowEdge[iBin] + width*i;
      if (debug)  std::cout<<"lowEdgeSlim[iBin*10 + i] = "<< iBin*10 + i <<" :: "<< lowEdgeSlim[iBin*10 + i]<<std::endl;
      if (debug)  std::cout<<"slimNBIN = " <<slimNBIN <<std::endl;
   }
       
  }
  
//    for (int iEdge = 0; iEdge < v_lowEdgeSlim.size(); iEdge++){
//     lowEdgeSlim[iEdge] = static_cast<float> (v_lowEdgeSlim.at(iEdge));
//    }
...
//       response.Miss (xt);
  ...
  TH1D* hMeasFJet_slim = new TH1D ("hMeasFJet_slim", "Measured FJet_slim", slimNBIN , lowEdgeSlim);
  TH1D* hMeasCJet_slim = new TH1D ("hMeasCJet_slim", "Measured CJet_slim", slimNBIN , lowEdgeSlim);
  ...
  for (Int_t iEvt= 0; iEvt<TreeTest->GetEntries(); iEvt++) {
   ...
   if (S_FJet_Pt>0 && S_CJet_Pt>0) {
    ...
    hMeasFJet_slim->Fill(S_FJet_Pt);
    hMeasCJet_slim->Fill(S_CJet_Pt);
  ...
  hMeasFJet->Sumw2();
  hMeasCJet->Sumw2();
  hMeasFJet_slim->Sumw2();
  hMeasCJet_slim->Sumw2();
  ...
  for (int iBinX = 0; iBinX<slimNBIN; iBinX++){
   ...
   hMeasFJet_slim->SetBinContent(iBinX+1,hMeasFJet_slim->GetBinContent(iBinX+1) / (lowEdgeSlim[iBinX+1] - lowEdgeSlim[iBinX]) / lumi);
   hMeasCJet_slim->SetBinContent(iBinX+1,hMeasCJet_slim->GetBinContent(iBinX+1) / (lowEdgeSlim[iBinX+1] - lowEdgeSlim[iBinX]) / lumi);       
...
//  gPad->BuildLegend();
 ...
 // gPad->BuildLegend();
  ...
  TCanvas* ccFJetMeasSlim = new TCanvas((TString) Form("ccFJetMeasSlim%s%s", nameTypeFileA.c_str(),nameTypeFileB.c_str()),(TString) Form("ccFJetMeasSlim%s%s", nameTypeFileA.c_str(),nameTypeFileB.c_str()),600,600);  

  hMeasFJet_slim->DrawCopy();
  ...
  hMeasFJet->Write();
  hMeasCJet->Write();
  hMeasFJet_slim->Write();
  hMeasCJet_slim->Write();
