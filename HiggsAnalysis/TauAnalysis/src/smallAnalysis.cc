...
smallAnalysis::smallAnalysis(const edm::ParameterSet& iConfig)
{
  ...
  thefile = new TFile("Prova_Zemu.root","recreate");
  thefile->cd();
...
void
smallAnalysis::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
   using namespace edm;
   ...
   for ( HepMC::GenEvent::particle_iterator p = myGenEvent->particles_begin();
	 ...
	 {
	   if ((abs((*aDaughter)->pdg_id())==11 ) || (abs((*aDaughter)->pdg_id())==13) ) {
	     isZ = true;
	     if ( abs((*aDaughter)->pdg_id()) == 11 ) {
	       MCp1[0]=(*aDaughter)->momentum().x();
	       MCp1[1]=(*aDaughter)->momentum().y();
	       MCp1[2]=(*aDaughter)->momentum().z();
	       MCeta1=(*aDaughter)->momentum().pseudoRapidity();
	       MCphi1=(*aDaughter)->momentum().phi();
	       pid1=(*aDaughter)->pdg_id();	    
	     }
	     if ( abs((*aDaughter)->pdg_id()) == 13 ) {
	       MCp2[0]=(*aDaughter)->momentum().x();
	       MCp2[1]=(*aDaughter)->momentum().y();
	       MCp2[2]=(*aDaughter)->momentum().z();
	       MCeta2=(*aDaughter)->momentum().pseudoRapidity();
	       MCphi2=(*aDaughter)->momentum().phi();
	       pid2=(*aDaughter)->pdg_id();	    
	     }
	   }
	 ...
	 MCpV[1]=(*p)->momentum().y();
	 MCpV[2]=(*p)->momentum().z();
	 MCphiV=(*p)->momentum().phi();
	 MCetaV=(*p)->momentum().pseudoRapidity();//
	 pidV=(*p)->pdg_id();
       }
     }
   }
   ...
   for( ele = electrons->begin(); ele != electrons->end() && nEleCand<ELEMAX; ++ ele ) {
     ...
     newtheta = atan(R_ecal/(ele->superCluster()->z()-ele->track()->vz()));
	 ...
	 if (R < 0.2 ) { PTr02+=trk->pt() ;}
	 if (R < 0.35) { PTr035+=trk->pt();}
	 if (R < 0.5 ) { PTr05+=trk->pt() ;}
       }
...
// save the event in a smaller tree:
   ...
   if (isZ) smalltree->Fill();
}


// ------------ method called once each job just before starting event loop  ------------
void 
smallAnalysis::beginJob(const edm::EventSetup&)
{
}

// ------------ method called once each job just after ending the event loop  ------------
void 
smallAnalysis::endJob() {
  std::cout << "++++++++++++++++++++++++++++++++++++++" << std::endl;
