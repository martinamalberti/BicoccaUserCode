// $Id: VBFElePlots.cc,v 1.6 2008/02/02 11:46:29 tancini Exp $
                             ...
                             const edm::EventSetup& iSetup)
{
  ...
  */
  ...
  reco::PixelMatchGsfElectronCollection::const_iterator first ;
  reco::PixelMatchGsfElectronCollection::const_iterator second ;
  double firstMaxPt = 0 ;
  double secondMaxPt = 0 ;
  
  reco::PixelMatchGsfElectronCollection::const_iterator eleID_first ;
  reco::PixelMatchGsfElectronCollection::const_iterator eleID_second ;
  ...
  for (reco::PixelMatchGsfElectronCollection::const_iterator eleIt = GSFHandle->begin () ; 
       ...
       ++eleIt) 
    {
      ...
      m_GSFhistos.m_pt->Fill (eleIt->energy ()) ;
      ...
      if (electronIDAssocItr != eleIdHandle->end ())
        {
          ...
          m_eleIDhistos.m_pt->Fill (eleIt->energy ()) ;
  ...
  if (secondMaxPt * firstMaxPt == 0) return ;

  //PG fill the GSF histos
  m_GSFhistos.m_deltaEta->Fill (first->eta () + second->eta ()) ;
  ...
  m_eleIDhistos.m_deltaEta->Fill (eleID_first->eta () + eleID_second->eta ()) ;
...
void VBFeleHistos::init (std::string tag, 
                         edm::Service<TFileService> & fs) 
{
  m_deltaEta   = fs->make<TH1F> (TString (tag.c_str ()) + "_deltaEta" ,"#Delta#eta between electrons",50,-1,1) ;
  m_deltaPhi   = fs->make<TH1F> (TString (tag.c_str ()) + "_deltaPhi" ,"#Delta#phi between electrons",50,-1,1) ;
  m_averageEta = fs->make<TH1F> (TString (tag.c_str ()) + "_averageEta" ,"average #eta of electrons",340,-3,3) ;
  m_averagePhi = fs->make<TH1F> (TString (tag.c_str ()) + "_averagePhi" ,"average #phi of electrons",360,0,6.28) ;
  m_invMass    = fs->make<TH1F> (TString (tag.c_str ()) + "_invMass" ,"M_{inv} of electrons",100,0,1000) ;
  m_eta        = fs->make<TH1F> (TString (tag.c_str ()) + "_eta" ,"#eta of electrons",340,-3,3) ;
  m_phi        = fs->make<TH1F> (TString (tag.c_str ()) + "_phi" ,"#phi of electrons",360,0,6.28) ;
  m_pt         = fs->make<TH1F> (TString (tag.c_str ()) + "_pt" ,"p_{T} of electrons",100,0,500) ;
  m_ptMax      = fs->make<TH1F> (TString (tag.c_str ()) + "_ptMax" ,"p_{T} of the max p_{T} electron",100,0,500) ;
  m_ptMin      = fs->make<TH1F> (TString (tag.c_str ()) + "_ptMin" ,"p_{T} of the min p_{T} electron",100,0,500) ;
}                         
