...
$OUTPUTTAG = $ARGV[2] ; # nome comune per il .cfi per il .cfg per il .csh per il .root
$FILEPERNODO = $ARGV[3] ; # numero file che si vogliono lanciare su un nodo
...
for ($MYRUN = 0; $MYRUN < $NUMFILES; $MYRUN++ ) 
{
	...
	{
		$COPIA = $OUTPUTTAG."_copia".$MIN.".csh"; # per la copia dei dati sul nodo
		system ("rm -f ".$COPIA ) ;  
	...
	$RIGA =`./goToRow.py numberedFileList.txt $MYRUN`; # sriptino di Danilo per leggere una riga di un file
	chop ($RIGA);

	print COPIANAME "cp ".$RIGA." $COPIAPATH\n"; 	# per la copia sul singolo nodo
...
for ($INDEX = 0; $INDEX < $NUMFILES; $INDEX = $INDEX + $FILEPERNODO)
{
	...
	$SAMPLEFILE = $OUTPUTTAG."_listaFiles_From".$INDEX.".cfi";
    ...
    $JOB = $OUTPUTTAG."_lanciaFiles_From".$MIN.".csh";
    $COPIA = $OUTPUTTAG."_copia".$MIN.".csh";
