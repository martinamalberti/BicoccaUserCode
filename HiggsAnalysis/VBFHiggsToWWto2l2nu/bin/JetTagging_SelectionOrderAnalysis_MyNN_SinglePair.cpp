  ...
  ///---- Pt ----
   ...
   for (int l=0; l<otherJets->GetEntries (); l++ ){ //--- loop over otherJets ----
    ...
    //---- Check if DEta and Mjj selections are fullfilled
     ...
     float etaA = myparticle_1->Eta();
     TLorentzVector* myparticle_1_temp = (TLorentzVector*) otherJets->At(maxNum1);
   ...
   if (tagJets_PT.first != -1 && tagJets_PT.second != -1){
    TLorentzVector* myRecoJet_1 = (TLorentzVector*) otherJets->At(tagJets_PT.first);   
    TLorentzVector* myRecoJet_2 = (TLorentzVector*) otherJets->At(tagJets_PT.second);
