#include "HiggsAnalysis/VBFHiggsToVV/interface/VBFMCDumper.h"
...
VBFMCDumper::VBFMCDumper(edm::Handle<reco::GenParticleCollection>& genParticles,
                   const int& eventType,
 ...
 {
  if(verbosity_p)
    std::cerr << "*** VBFMCDumper::ctor ***" << std::endl;
...
{
  if(verbosity_p)
    std::cerr << "*** VBFMCDumper::dtor ***" << std::endl;
}
...
void VBFMCDumper::Analyze(edm::Handle<reco::GenParticleCollection>& genParticles)
{
  if(verbosity_p)
  ...
  if(!mcH_p)
  {
    std::cerr << ">>> VBFMCDumper::Analyze::Warning: no H in the event" << std::endl;
  ...
  if(VBuffer.size() != 2)
  {
     std::cerr << "VBFMCDumper::Analyze::Warning: not a H -> VV decay" << std::endl;
  ...
  {
     std::cerr << "VBFMCDumper::Analyze::Warning: not a V1 -> ff decay" << std::endl;
  ...
  {
     std::cerr << "VBFMCDumper::Analyze::Warning: not a V2 -> ff decay" << std::endl;
...
void VBFMCDumper::PrintEventInfo()
{
  std::cout << "\n>>> VBFMCDumper::PrintHInfo \n" << std::endl;
...
void VBFMCDumper::GetHDecayMode()
{
...
void VBFMCDumper::GetVDecayMode(const int& Vindex)
{
